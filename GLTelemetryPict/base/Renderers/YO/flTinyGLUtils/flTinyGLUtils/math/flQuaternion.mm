...
void Quaternion::get(Matrix4x4& matrix) const {
...
void Quaternion::makeRotate( float angle, const Vec4f& vec ) {
	makeRotate( angle, vec.x, vec.y, vec.z );
}


void Quaternion::makeRotate ( float angle1, const Vec4f& axis1,
 ...
 */
void Quaternion::makeRotate( const Vec4f& from, const Vec4f& to ) {
	...
	Vec4f sourceVector = from;
	Vec4f targetVector = to;
	...
	if (dotProdPlus1 < 1e-7) {
		...
		const Vec4f tmp = sourceVector.getCrossed(targetVector) / (2.0 * s);
		_v.x = tmp.x;
		_v.y = tmp.y;
		_v.z = tmp.z;
		_v.w = s;
...
void Quaternion::makeRotate_original( const Vec4f& from, const Vec4f& to ) {
	const float epsilon = 0.0000001f;
	...
	if ( fabs(cosangle - 1) < epsilon ) {
		...
		if ( fabs(cosangle + 1.0) < epsilon ) {
			...
			if (fabs(from.x) < fabs(from.y))
				if (fabs(from.x) < fabs(from.z)) tmp.set(1.0, 0.0, 0.0, 0.0); // use x axis.
				else tmp.set(0.0, 0.0, 1.0, 0.0);
			else if (fabs(from.y) < fabs(from.z)) tmp.set(0.0, 1.0, 0.0, 0.0);
			else tmp.set(0.0, 0.0, 1.0, 0.0);

			Vec4f fromd(from.x, from.y, from.z);

			// find orthogonal axis.
			Vec4f axis(fromd.getCrossed(tmp));
			axis.normalize();
			...
			Vec4f axis(from.getCrossed(to));
			float angle = acos( cosangle );
			makeRotate( angle, axis );
		}
}

void Quaternion::getRotate( float& angle, Vec4f& vec ) const {
	float x, y, z;
...
// with a rotation!
void Quaternion::getRotate( float& angle, float& x, float& y, float& z ) const {
...
void Quaternion::slerp( float t, const Quaternion& from, const Quaternion& to ) {
...
Vec4f Quaternion::getEuler() const {
	float test = x()*y() + z()*w();
	float heading;
	float attitude;
	float bank;
	...
	return Vec4f(radToDeg(attitude), radToDeg(heading), radToDeg(bank));
}
