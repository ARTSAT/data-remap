...
ScopedBlending::ScopedBlending(BlendMode mode)
{
...
ScopedColor::ScopedColor(const FloatColor& color)
{
    glPushAttrib(GL_COLOR_BUFFER_BIT);
    setColor(color);
}

ScopedColor::ScopedColor(const Color& color, float alpha)
{
    glPushAttrib(GL_COLOR_BUFFER_BIT);
    setColor(color, alpha);
}

ScopedColor::ScopedColor(const FloatColor& color, float alpha)
{
    glPushAttrib(GL_COLOR_BUFFER_BIT);
    setColor(color, alpha);
}

ScopedColor::ScopedColor(float w, float a)
{
    glPushAttrib(GL_COLOR_BUFFER_BIT);
    setColor(w, a);
}
...
{
    setColor(color.r / 255.f, color.g / 255.f, color.b / 255.f, color.a / 255.f);
}
...
{
    setColor(color.r / 255.f, color.g / 255.f, color.b / 255.f, alpha);
}
                           ...
                           bool flip)
{
    ...
    perspective->makePerspectiveMatrix(fov, aspect, nearDist, farDist);
    
    lookat->makeLookAtViewMatrix(Vec4f(eyeX, eyeY, dist, 1),
                                 Vec4f(eyeX, eyeY, 0, 1),
                                 Vec4f(0, 1, 0, 1));
    if (flip) {
        lookat->glTranslate(0.f, h, 0.f);
        lookat->glScale(1.f, -1.f, 1.f);
    }
}
                     ...
                     float farDist)
{
    ...
    *ortho = Matrix4x4::newOrthoMatrix(0,
                                       viewW,
                                       viewH,
                                       0,
                                       nearDist,
...
#pragma mark ___________________________________________________________________
void billboard(Matrix4x4& mat, bool flattenScale)
{
    if (flattenScale) {
        mat(0, 0) = mat(1, 1) = mat(2, 2) = 1.f; // scale
    }
    mat(1, 0) = mat(2, 0) = mat(0, 1) = mat(2, 1) = mat(0, 2) = mat(1, 2) = 0.f; // rotation
}

ScopedLineWidth::ScopedLineWidth(float w)
{
...
ScopedPointSize::ScopedPointSize(float s)
{
    glPushAttrib(GL_POINT_BIT);
    glPointSize(s);
}

ScopedPointSize::~ScopedPointSize()
{
    glPopAttrib();
}

#ifndef TARGET_OPENGLES
void billboard(bool normalizeScale)
{
    GLdouble m[16];
                           ...
                           bool flip)
{
    ...
    makeScreenPerspective(&projection,
                          &modelview,
                          w,
                          h,
                          fov,
...
void loadOrthoMatrix(float w, float h, float nearDist, float farDist)
{
    Matrix4x4 ortho;
    makeScreenOrtho(&ortho, w, h, nearDist, farDist);
